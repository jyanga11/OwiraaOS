...
void * kmalloc(uint32_t bytes) {
    ...
    // Add size of metadata to the total number of bytes for the request
    bytes += sizeof(heap_segment_t);
    bytes += bytes % 16 ? 16 - (bytes % 16) : 0;

    // Find a heap segment to allocate (Best Fit alg)
    ...
    // If an allocation is not found return NULL
    if (best == NULL) {
	return NULL;
    }

    // If segment is excessively large, split it
